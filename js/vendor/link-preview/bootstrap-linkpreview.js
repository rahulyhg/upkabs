    ...
    LinkPreview.prototype = {
        ...
        init: function(element, options) {
            ...
            if (options && options.refreshButton) {
                ...
                var that = this;
                this.$refreshButton.on("change", function() {
                    // Change HTTPS to HTTP
                    var changeUrl = $('#link').val().replace(/^https:\/\//i, 'http://')
                    $('#link').val(changeUrl);
        ...
        getSource: function(url, onSuccess, onError) {
            ...
            if (!this.validateUrl(this.url)) {
                this.$previewContainer.html('No content available');
                return;
            }
        ...
        renderPreview: function(url, data, that) {            
            ...
            if (image) {
                var $image = $('<img class="media-object post-image">').attr("src", image),
                    $spanLeft = $('<div id="image-link-preview"></div>').addClass("media-left");
                $spanRight = $('<div id="text-link-preview"></div>').addClass("media-body");
                $spanLeft
                    .append($image);
            ...
            // Set Input Variable Form
            $('#input-title').val(title);
            $('#input-text').val(description);
            $('#input-media').val(image);
            $('#input-refrence').val(url);
        ...
        findImageInDom: function($dom) {
            var imageSrc = $dom.find("meta[itemprop=image]").attr("content") ||
                ...
                $dom.find("meta[property='og:image']").attr("content") ||
