var GetMostCommentedPosts = (function() {
	"use strict";
	...
	module.initData = function () {
		// First State Without Click
		module.sendRequestData();
		module.sendRequestPaginationData();

		// With Click
		mostCommented.on('click', function(event) {
	...
	module.sendRequestData = function (page) {
		...
		// Ajax API
		$.ajax({
			url: Incom.API.trending.get+'/commented/'+page
		})
		.done(
	...
	module.onRequestDataDone = function (response) {
		...
		// Flag The Button
		mostCommented.addClass('btn-acted');
